...
class BotActivity {
  constructor() {
    ...
    for (let i = 0; i < Math.floor(this.activities.length / this.ratf - 0.5); i++) {
      let index = (i + 1) * this.ratf + i;

      let repActvts = Array.from(this.activities.filter(x => x.several === true));
      repActvts.forEach(x => {
  ...
  activities = [
    BotActivity.MakeActObj("How I'm disassembled and reassembled differently",
      1.1, 'WATCHING', true),
    BotActivity.MakeActObj('with my vodka bottle'),
    BotActivity.MakeActObj('𝔀𝓲𝓽𝓱 𝓯𝓵𝓸𝔀𝓮𝓻𝓼'),
    BotActivity.MakeActObj(' ʍıʇɥ ɹǝɐlıʇʎ'),
    BotActivity.MakeActObj("Jesus Christ, that's a pretty face",
      this.SongLyricInterval)
  ];

  /* eslint-disable class-methods-use-this */
  ChangeActivity(ActivityClass) {
    let cls = ActivityClass;
    let activity = ActivityClass.activities[cls.iteration];
