...
import givCmd from '../src/commands/giveaways/change-giveaway-channel.js';
...
function FailSimpleFetch(count) {
  for (let index = 0; index < count; index++)
    mockSimpleFetch.mockRejectedValueOnce(
      `expect ${index === 0 ? 'no' : index} more failures for giveaways`
    );
  return count + 1;
}
// MOCK END
...
describe('giveaway interactions', () => {
  ...
  afterEach(() => {
    ...
    for (let index = 0, n = messagesBefore.length; index < n; index++) {
      channels[index].messages.cache = messagesBefore[index];
    }
  });
